...
struct kobj_attribute my_firm_attr = __ATTR(dec, 0660, my_sysfs_show, my_sysfs_store);
// ---- sysfs related ----
...
static ssize_t my_sysfs_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	...
	return sprintf(buf, "%d %d", sysfs_int,dec);
...
static int __init my_mod_init(void)
{
	...
	kobj_ref = kobject_create_and_add(MY_SYSFS_DIR, firmware_obj);
	if (kobj_ref==NULL)
	{
	...
	if (sysfs_create_file(kobj_ref, &my_firm_attr.attr))
	{
		pr_err("Cannot create sysfs file\n");
		goto r_my_firm_attr;
	}
...
static void __exit my_mod_exit(void)
{
	sysfs_remove_file(firmware_obj, &my_sysfs_attr.attr);
	sysfs_remove_file(firmware_obj, &my_firm_attr.attr);
