...
struct my_node *node,*node1,*node3, *tmp;
// function prototypes
...
static ssize_t my_write(struct file *file, const char __user *buf, size_t len, loff_t *off)
{
	...
	char c;
		// kmalloc node
		node1 = (struct my_node *)kmalloc(sizeof(struct my_node), GFP_KERNEL);
		node2 = (struct my_node *)kmalloc(sizeof(struct my_node), GFP_KERNEL);
		node3 = (struct my_node *)kmalloc(sizeof(struct my_node), GFP_KERNEL);
